...
get.outputs = function(n=100, subnetSize.X=rep(10,2), subnetSize.E=rep(10,2),
                       sparsity.B=5, sparsity.Theta=5, K=5, seed.vec=1:50, filename=NULL){
  ...
  loopfun = function(rep){
    ...
    ## get all models
    loopfun1 = function(m){
      jmmle(Y.list, Y.indices, X.list, B.group.array=B0.group.array, Theta.groups=Theta.groups,
            lambda = lambda.vec[m],
  ...
  if(is.null(filename)){
    filename = paste0("estfull_n",n,"p",p,"q",q,"_1.Rda")
  }
...
##### Generate data
systme.time(
  get.outputs(n = 100, subnetSize.E = c(30, 30), subnetSize.X = c(15, 15))
)