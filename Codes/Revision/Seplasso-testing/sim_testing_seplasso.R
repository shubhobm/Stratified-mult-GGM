...
get.outputs = function(n=100, subnetSize.X=rep(10,2), subnetSize.E=rep(10,2),
                       sparsity.B=5, sparsity.Theta=5, K=2, seed.vec=1:150, filename=NULL){
  ...
  loopfun = function(seed){
    ...
    loopfun.B = function(nlam){
      ...
      for(k in 1:K){
        for(j in 1:q){
            jmod = glmnet(X, Y[,j], intercept=FALSE)
            B.array[,j,k] = predict(jmod, s=lam, type="coefficients")[-1]
        }
        bic.vec[nlam] = bic.vec[nlam] +
          sum((Y.list[[k]] - X.list[[k]] %*% B.array[,,k])^2) +
  ...
  if(is.null(filename)){
    filename = paste0("testsep_n",n,"p",p,"q",q,".Rda")
  }
...
get.outputs(n = 100, subnetSize.X = c(100, 100), subnetSize.E = c(100, 100),
            sparsity.B=30, sparsity.Theta=30, filename="testsep_n100p200q200modelB.Rda")
get.outputs(n = 200, subnetSize.X = c(100, 100), subnetSize.E = c(100, 100),
            sparsity.B=30, sparsity.Theta=30, filename="testsep_n200p200q200modelB.Rda")
get.outputs(n = 300, subnetSize.X = c(100, 100), subnetSize.E = c(100, 100),
            sparsity.B=30, sparsity.Theta=30, filename="testsep_n300p200q200modelB.Rda")
