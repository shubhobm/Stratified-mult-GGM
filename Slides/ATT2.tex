...
\visible<2->{\2 {\it Worked on diverse data applications: climate data, fMRI, genomics etc.}}

\vspace{1em}
\1 Current research: Statistical machine learning- complex graphical models,
\visible<3->{\2 {\it Motivation: modelling multiple Omics data.}}

\vspace{1em}
\1 Future plans
\visible<4->{\2 {\colbit Extending methods to spatial models;}}
\visible<5->{\2 {\it {\colb Uncertainty quantification in complex high-dimensional systems}: causal models, tensor models.}}
\visible<6->{\2 {\colbit The statistics of artificial intelligence: theory and interpretability.}}
\end{outline}
\end{frame}
...
\visible<2>{
\begin{center}
\includegraphics[height=.5\textheight]{BetaTheta0}
\end{center}
}}
\only<3>{
\begin{center}
\includegraphics[height=.5\textheight]{BetaTheta1}
\end{center}
}
\only<4>{
\begin{center}
\includegraphics[height=.5\textheight]{BetaTheta2}
\end{center}
}
\only<5>{
\begin{center}
\includegraphics[height=.5\textheight]{BetaTheta3}
\end{center}
}
...
\begin{itemize}
\visible<2->{
\item Lasso: $\widehat \bfbeta = \argmin_\bfbeta \| \bfy - \bfX \bfbeta \|^2 + \| \bfbeta \|_1$;
}
...
%
\begin{itemize}
\visible<2->{\item Take the multi-layer structure as a {\colbit generative model}.}
\vspace{1em}

\visible<3->{\item Only the top layer is observed, other layers are composed of latent variables.}
