...
The node-wise regression step in the JSEM model \eqref{eqn:jsem-model} uses a Bayesian Information Criterion (BIC) for tuning parameter selection  \citep{MaMichailidis15}. The step for updating $\Theta$, i.e. \eqref{eqn:update-Theta}, in our JMMLE algorithm is analogous to this procedure, hence we use BIC to select the penalty parameter $\gamma_n$. In our setting the BIC for a given $\gamma_n$ and fixed $\cB$ is given by:
%
\begin{align*}
\text{BIC} (\gamma_n; \cB) &=
...
We use the High-dimensional BIC (HBIC, \citet{KimKwonChoi12, WangKimLi13}) to select the other tuning parameter, $\lambda$:
%
\begin{align*}
...
\end{align*}
%
Unlike BIC, the penalty term in HBIC scales with the parameter dimensions. As a result, the tuning poarameter selected as the minimizer of HBIC asymptotically identifies the oracle estimator in ultra-high dimensional penalized problems \citep{FanTang13, WangKimLi13}. In our case, we choose an optimal $\lambda_n$ as the empirical minimizer of HBIC by training multiple JMMLE models using Algorithm \ref{algo:jmmle-algo} over a finite set of values $\lambda_n \in \cD_n$: 
$
