...
We start with a lemma to help along the proof.
...
\noindent Applying Lemma~\ref{lemma:omega-diff} it is immediate that
%
...
$$
%
Taking $\epsilon = n^{-1/4}$, we have $s_i \geq \sigma_{x,i,-i} - n^{-1/4}$ with probability $\geq 1 - \kappa_i n^{-1/2}$. Then, for $n$ satisfying \eqref{eqn:ThmTestingProofeq32} and $\sigma_{x,i,-i} - n^{-1/4} >  D_\zeta \sqrt{ V_x}$, we get the bound with the above probability:
%
\begin{align}\label{eqn:ThmTestingProofeq34}
\frac{1}{\widehat s_i} \leq \frac{1}{\sigma_{x,i,-i} - n^{-1/4} - D_\zeta \sqrt{ V_x}}
\end{align}
%
...
$$
%
It is easy to prove that $\bfA^{1/2} + \sqrt \delta \bfJ_a \geq (\bfA_1 + \delta \bfJ_a)^{1/2}$ and the same for $\bfA_1$. The lemma follows.
\end{proof}
