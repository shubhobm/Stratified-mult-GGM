...
%\end{align}
%
We choose the initial values $\{ \widehat \bfB^{k (0)} \}$ by fitting separate lasso regression models for each column of the coefficient matrices:
%
\begin{align}\label{eqn:init-B}
...
\vspace{1em}
\noindent{\bf (E1)} The matrices $(\bfT^k)^2, k \in \cI_K$ are diagonally dominant, i.e. %
%
$$
|t_{0,jj}^k | > \sum_{j' \neq j} |t_{0,jj'}^k |
$$
...
\vspace{1em}
\noindent{\bf (E2)} For $k \in \cI_K$, $\Omega_{y0}^k$ is diagonally dominant, i.e.
%
$
|\omega_{y0,jj} | > \sum_{j' \neq j} |\omega_{y0,jj'} |
$
...
\noindent{\bf (E3)} There exist constants $c_0, d_0$ such that for $k \in \cI_K$,
%
\[
