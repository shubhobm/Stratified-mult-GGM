...
%
$$
H_0^{ij}: b_{0 ij}^1 - b_{0ij}^2 = 0 \quad \text{vs.} \quad H_1^{ij}: b_{0 ij}^1 - b_{0 ij}^2 \neq 0 
$$
%
...
$$
%
For a pre-specified level $\alpha$, we now choose a threshold that ensures both FDP and FDR $\leq \alpha$ using the framework of \cite{Liu17} and \cite{Efron07}. To do this we define the following:
%
\begin{align*}
P_0 = 2 \Phi (1) - 1; \quad
...
\end{align*}
%
where $\Phi(\cdot)$ and $\phi(\cdot)$ are the standard normal distribution and density functions, respectively. The procedure for FDR control is now given by Algorithm \ref{algo:AlgoFDR}.
...
%
$$
\hat \tau = \inf \left\{\tau \in \BR: 1 - \Phi(\tau) \leq \frac{\alpha}{q}
A(\tau) \max \left( \sum_{j \in \cI_q} \BI( |d_{ij}| \geq \tau), 1 \right) \right\}
$$
%
...
Under some regularity conditions, this procedure maintains FDR and FDP asymptotically at a pre-specified level $\alpha \in (0,1)$.

\begin{Theorem}\label{thm:FDRthm}
Assume the following conditions:
{\colrbf tbd}

Then the following hold: (i) $A(\hat \tau) \stackrel{P}{\rightarrow} 1$, (ii) $FDP(\hat \tau) \stackrel{P}{\rightarrow} \alpha$ and $\lim_{(n,p) \rightarrow \infty} FDR(\hat \tau) \rightarrow \alpha$.
\end{Theorem}
...
The proof follows that of Theorem~\ref{thm:ThetaThm}, with a different group norm structure. We only point out the differences.
