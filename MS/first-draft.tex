...
\noindent{\bf(B2)} The deviation bound holds:
%
$$
\| \widehat \bfgamma - \widehat \bfGamma \bfbeta_0 \|_\infty \leq \BR( v_\Theta, \Sigma_y)
$$
%
{\colrbf tbd}

\noindent{\bf(B3)} $\widehat \bfGamma \sim RE(\psi^*, \phi^*)$ with $Kpq \phi^* \leq \psi^*/2$.

Then, given the choice of tuning parameter
%
$$
\lambda \geq 4 \sqrt{pq} \BR( v_\Theta, \Sigma_y)
$$
%
the following holds
%
...
The proof follows that of Theorem~\ref{thm:ThetaThm}, with a different group norm structure. We shall only point our the differences.
...
\end{align}
%
The bounds in (\ref{eqn:BetaThmEqn1}), (\ref{eqn:BetaThmEqn2}), (\ref{eqn:BetaThmEqn3}) and (\ref{eqn:BetaThmEqn4}) follow.
...
%with probability $ \geq 1 - 1/p^{\tau_1-2}, \tau_1 > 2$.
%
\item $\widehat \bfGamma$ satisfies the RE condition: $ \widehat \bfGamma \sim RE (\psi^*, \phi^*)$, where 
%
$$
{\colrbf tbd}
$$
%
with probability ${\colrbf tbd}$.
%
...
%
$$
\| \widehat \bfgamma - \widehat \bfGamma \bfbeta_0 \|_\infty \leq {\colrbf tbd}
$$
%
