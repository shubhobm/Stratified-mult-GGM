...
\noindent{\bf(T3)} Define \textbf{$\widehat \bfS^k := (\bfY^k - \bfX^k \widehat \bfB^k)(\bfY^k - \bfX^k \widehat \bfB^k)^T/n, k \in \cI_K$}. Then for all $j \in \cI_q$, $\left\| \widehat \bfS^k \bfT_{0,j}^k \right\|_\infty \leq q( v_\beta) $ where $q( v_\beta) = O(1)$ is some quantity that depends on $v_\beta$.

\noindent{\bf(T4)} $\widehat \bfS^k \sim RE(\bar \psi_y^k, \bar \phi_y^k)$ with {\colrbf tbd};

\noindent{\bf(T5)} Assumption (A2) holds for $\widehat \bfE^k$. Then
...
Consider the estimation problem in (\ref{eqn:EstEqn2}) and choose $\lambda_{jj'}^{[g]}$ as in Proposition~\ref{lemma:LemmaE2}. Given the condition {\colrbf tbd} holds, on the event $\cA$ we shall have
$$
\sum_{j \neq j'} \sum_{g \in \cG_y^{jj'}} \lambda_{jj'}^g \| \widehat \bftheta_{jj'}^{[g]} - \bftheta_{0,jj'}^{[g]}\| \leq {\colrbf tbd}
$$
{\colrbf tbd}
\end{Proposition}
...
%
\begin{align}
\sum_{k=1}^K (\bfd_j^k)^T \hat \bfS^k \bfd_j^k & \leq - 2 \sum_{k=1}^K (\bfd_j^k)^T \hat \bfS^k \bfT_j^k + \sum_{j \neq j', g \in \cG_y^{jj'}} \lambda_{jj'} \left( \| \bfT_{jj'}^{[g]} \| -  \| \bfT_{jj'}^{[g]} + \bfd_{jj'}^{[g]}\| \right)
\end{align}

%
with $\widehat \bfT_j^k := \bfT_{0,j}^k + \bfd_{j}^k$ etc. The $k^\text{th}$ summand in the first term on the right hand side is $\leq 2 \| \bfd_j^k \|_1 q( v_\beta)$ by assumption (T3). For the second term, suppose $\cS_{0,j}$ is the support of $\bfT_j$, i.e. $\cS_{0,j} = \{ (j',g): T_{jj'}^{[g]} \neq 0 \}$. Then
%
\begin{align*}
...
The right hand side is $\leq \lambda_n \| \bfD^k \|_1$ with $\bfD^k = (\bfd_j^k)_{j \in \cI_q}$.
...
%
\item $\widehat \bfS^k$ satisfies the following bound for any $j \in \cI_q$
%
$$
...
$$
with probability $\geq 1 - 1/p^{\tau_1-2} - 6c_1 \exp [-(c_2^2-1) \log(pq)]$.
%
\end{enumerate} 
\end{Proposition}
...
\end{Lemma}
%
This is same as Lemma 2 in Appendix B of \cite{LinEtal16} and its proof can be found there. Now denote $\widehat \bfE^k = \bfY^k - \bfX^k \widehat \bfB^k$. For $\bfv \in \BR^q$, we have
%
...
\end{Lemma}
%
This is a part of Lemma 3 of Appendix B in \cite{LinEDtal16}, and is proved therein. Here we take $\bfX \equiv \bfX^k, \bfE \equiv \bfE^k$, and subsequently collect all summands in (\ref{eqn:ErrorREeqn3}) get
%
$$
...
$$
with probability $\geq 1 - 2\exp(- c_3 n) - 6c_1 \exp [-(c_2^2-1) \log(pq)]$. This concludes the proof of part 2.

For part 3, we have the following:
%
\begin{align}
