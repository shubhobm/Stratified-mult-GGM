...
\noindent{\bf(T1)} $\| \widehat \bfB^k - \bfB^k_0 \|_F \leq v_\beta$;

\noindent{\bf(T2)} $\| \bfX^k (\widehat \bfB^k - \bfB^k_0 ) \|_F \leq c(v_\beta)$ for some non-negative function $c(.)$;
...
%
$$
\lambda_{jj'}^g \geq \max_{k \in g} \frac{2}{\sqrt{n \omega_{jj}^k}} \left( \sqrt{|g| (1 + c(v_\beta)) } + \frac{\pi}{\sqrt 2} \sqrt {r \log G_0} \right)
$$
%
...
By cauchy-schwarz inequality, $| (\bfQ_{j'}^k)^T \bfdelta_j^k | \leq \| \bfdelta_j^k \|$. Now $T_{jj}^k = 1$, and for $j \neq j'$, $|T_{jj'}^k| =  |\theta_{jj'}^k| = |\omega_{y,jj'}^k| / \omega_{y,jj}^k \leq 1$ since $\Omega_y^k$ is diagonally dominant. Thus $\| \bfdelta_j^k \| \leq \| \bfX^k (\bfB_0^k - \widehat \bfB^k) \|_F$. Hence by assumption (T2),
%
$$
\| \widehat \bfzeta_{ij}^{[g]} \| \leq \frac{1}{\sqrt n} ( \| \bfZ^{[g]} \| + |g| c(v_\beta))
$$
%
so that
$$
\BP ( \{ \cA_{ij}^g \}^c ) = \BP \left( \| \widehat \bfzeta_{ij}^{[g]} \| > \frac{\lambda_{ij}^g}{2} \right) \leq \BP \left( \| \bfZ^{[g]} \|  > \frac{\sqrt n \lambda_{ij}^g}{2} - |g| c(v_\beta)  \right)
$$
...
\begin{enumerate}
\item With probability $\geq {\colrbf tbd}$ we have $\| \bfX^k ( \hat \bfB^k - \bfB_0^k ) \| \leq c( v_\beta)$, where
%
$$
c(v_\beta) = {\colrbf tbd}
$$
%
...
\end{align}
%
For the first summand, $ \bfv^T \bfS^k \bfv \geq \psi_y \| \bfv \|^2 - \phi_y \| \bfv \|_1^2$ with $\psi_y = \Lambda_{\min} (\Sigma_y^k)/2, \phi_y = \psi \log p/n$ by applying Lemma \ref{lemma:ErrorRElemma1} on $\bfS^k$. The second summand is greater than or equal to 0. For the third summand,
%
$$
...
For zero-mean independent sub-gaussian matrices $\bfX \in \BM(n,p), \bfE = \in \BM(n,q)$ with parameters $(\Sigma_x, \sigma_x^2)$ and $(\Sigma_e, \sigma_e^2)$ respectively, given that $n \succsim \log(pq)$ the following holds for some $c_1 >0, c_2 > 1$:
%
$$
