...
$$
%
where $\BQ \left(v_\beta, \Sigma_x^k, \Sigma_y^k \right)$ is a deterministic function depending on the population parametes $\cB, \Sigma_x^k$ and $\Sigma_y^k$.

\noindent{\bf(T3)} Denote $\widehat \bfS^k = (\widehat \bfE^k)^T \widehat \bfE^k/n$. Then $\widehat \bfS^k \sim RE(\psi^k, \phi^k)$ with $K^2 q \phi \leq \psi/2$ where $ \psi = \min_k \psi^k, \phi = \max_k \phi^k $;

\noindent{\bf(T4?)} Assumption (A2) holds for $\widehat \Sigma_y^k$. Then, given the choice of tuning parameter
%
$$
...
{\colrbf O \left( \frac{48 \sqrt{S_0} c_0 \gamma_n }{\psi \sqrt K} \right)}
\end{align}
%
...
\textit{Part I.} The proof has three parts, where we prove the consistency of the neighborhood regression coefficients, selection of edge sets, and finally the refitting step, respectively. This is the same structure as the proof of Theorem 1 in \cite{MaMichailidis15}, where they prove the consistency of the JSEM estimates. However, many details are entirely different here.
...
The following proposition establishes error bounds for the estimated $y$-neighborhood coefficients.
...
(\psi - K^2 q \phi ) \| \bfD_j \|_F^2 \geq \frac{\psi}{2}  \| \bfD_j \|_F^2
$$
%
