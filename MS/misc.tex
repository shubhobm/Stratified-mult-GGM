...
Given conditions (T1) and (T2), the following holds for sample size $n$ such that $n \succsim \log (pq)$ and $\sqrt{\sigma_{x,i,-i}} - n^{-1/4} - D_\zeta \sqrt{V_x} > 0$:
%
$$
...
With the assumptions of Theorem~\ref{thm:PowerThm}, the following holds with probability $\geq 1 - 6c_{6} \exp [-(c_{7}^2-1) \log (p-1)] - 1/p^{\tau_2-2} - \kappa_i / \sqrt n, \tau_2 > 2$:
%
\begin{align}
\left| \frac{m_i^k}{\sqrt n} - \sqrt{ \sigma_{x0,i,-i}^k} \right| & \leq
D_i := \sqrt{ \frac{ \log 4 + \tau_2}{c_i^k n}} +
\frac{D_\zeta + \| \bfzeta_{0i} \|_1 }{\sqrt{ \sigma_{x0,i,-i}^k} - n^{-1/2} - D_\zeta \sqrt{V_x}} \times \notag\\
& \left[ c_{7} [(\sigma_{x0,i,-i}^k \Lambda_{\max} (\Sigma_{x0, -i}^k)]^{1/2} \sqrt{ \frac{\log p}{n}} + D_\zeta V_x^k \right], \label{eqn:PowerThmLemmaEqn}
\end{align}
%
where $c_i^k = [ 128 (1 + 4 \sigma_{x0,i,-i}^k)^2 (\sigma_{x0,i,-i}^k)^2 ]^{-1}, \kappa_i^k = \BV [(X_i^k - \BX_{-i}^k \bfzeta_{0,-i}^k)^2]$ for $k = 1,2$, and the sample size satisfies $n \succsim \log p$,  $n \geq 512 (1 + 4 \sigma_{x0,i,-i}^k)^42 (\sigma_{x0,i,-i}^k)^4 \log(4) $ and $\sqrt{ \sigma_{x0,i,-i}^k} - n^{-1/4} - D_\zeta \sqrt{V_x^k} > 0$.
\end{Lemma}
%
...
For the pooled covariance matrix estimate $\widehat \Sigma_i$, for sample size $n {\colrbf tbd}$ the following holds:
%
$$
\left\| \widehat \Sigma_i - \Sigma_i \right\|_\infty \leq {\colrbf tbd}
$$

with probability $\geq 1 - 2 \kappa_i/\sqrt n$.
\end{Lemma} 
