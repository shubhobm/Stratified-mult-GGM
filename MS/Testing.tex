...
+ \Delta_n \right)^{-1} \bfdelta \right) + o(1)
$$
%
where $\| \Delta_n \|_\infty = o(1)$, $K_q$ is the cumulative distribution function of the $\chi^2_q$ distribution, and $m_{0 i}^k$ is the population version of $m_i^k, k=  1,2$. Consequently, for $\| \bfdelta \|_\infty \geq {\colrbf tbd}$, $P( H_0^i \text{ is rejected }) \rightarrow 1$ as $(n,p,q) \rightarrow \infty$.
\end{Theorem}
{\colrbf proof tbd}
%
...
\noindent {\bf (D1)} Define $r_{0,jj'}^k = \sigma_{y0,jj'}^k /\sqrt{\sigma_{y0,jj}^k \sigma_{y0,j'j'}^k}$ for $j,j' \in \cI_q, k = 1,2$. Suppose there exists $0 < r < 1$ such that $\max_{1 \leq j < j' \leq q} | r_{0,jj'}^k | \leq r$, and for every $j \in \cI_q$,
%
$$
...
\noindent {\bf (D1*)} Suppose there exists $0 < r < 1$ such that $\max_{1 \leq j < j' \leq q} | r_{0,jj'}^k | \leq r$, and for every $j \in \cI_q$,
%
$$
