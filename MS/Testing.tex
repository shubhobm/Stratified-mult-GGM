...
\noindent {\bf (D1)} Define $r_{0,jj'}^k = \sigma_{0,jj'}^k /\sqrt{\sigma_{0,jj}^k \sigma_{0,j'j'}^k}$ for $j,j' \in \cI_q, k = 1,2$. Suppose there exists $0 < r < 1$ such that $\max_{1 \leq j < j' \leq q} | r_{0,jj'}^k | \leq r$, and for every $j \in \cI_q$,
%
$$
